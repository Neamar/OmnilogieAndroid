...
public class RechercheActivity extends SpecialActivity implements CallbackObject {
	/**
	 * ArrayList utilisé pour les items de la liste. Contient des objets
	 * Spanned, afin de décoder les entités HTML et de supprimer les balises
	 * inclues.
	 */
	private ArrayList<HashMap<String, Spanned>> resultats;

	/**
	 * Liste des URLs. Ces URLs sont tronquées de la partie commune
	 * "http://omnilogie.fr/O/" et ne contiennent plus que le titre sous forme
	 * d'URL.
	 * 
	...
	/**
	 * La listView qui affiche les résultats. Doit être stocké en propriété afin
	 * qu'on puisse y accéder depuis le thread.
	 */
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Accrocher un évènement lors d'un clic sur un élément de la liste afin
		// de lancer l'application Omnilogie
	...
	protected void lancerRecherche(String texteRecherche) {
		...
		// Restreindre au site Omnilogie.fr et même plus spécifiquement aux
		// articles
	...
	/**
	 * Méthode de callback utilisée pour traiter le JSON une fois celui-ci
	 * récupéré. On arrive à cette méthode après la fonction lancerRecherche().
	 * 
	 * @param objet
	...
	public void callback(Object objet) {
		...
		if (jsonDatas != null) {
			...
			try {
				// Lire le tableau.
				// Cf. https://developers.google.com/web-search/docs/#fonje pour
				// les détails
				JSONArray jsonArray = jsonDatas.getJSONObject("responseData").getJSONArray(
						"results");
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					resultat.put("titre", Html.fromHtml(Html.toHtml(
							decodeJson(jsonResult, "titleNoFormatting")).replace(
							"| Un article d'Omnilogie.fr", "")));
	...
	/**
	 * Renvoie un élément du JSON dans l'encodage correct, sous forme de
	 * Spanned.
	 * 
	 * @param jsonObject
	 *            l'objet dans lequel on souhaite lire
	 * @param key
	 *            la clé à récupérer
	 ...
	 */
	protected Spanned decodeJson(JSONObject jsonObject, String key)
			throws UnsupportedEncodingException, JSONException {
		return Html.fromHtml(new String(jsonObject.getString(key).getBytes("ISO-8859-1"), "UTF-8"));
	}

	/**
	 * Termine la préparation de la vue, une fois que les données distantes ont
	 * été récupérées.
	 */
	protected Runnable remplirUIAvecDatas = new Runnable() {
		...
		public void run() {
			...
			ListAdapter adapter = new SimpleAdapter(rechercheActivity, resultats,
					R.layout.item_recherche_resultat, new String[] { "titre", "fragment" },
					new int[] { R.id.item_recherche_resultat_titre,
							R.id.item_recherche_resultat_fragment });
