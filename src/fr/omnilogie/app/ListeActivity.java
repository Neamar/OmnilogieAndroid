...
public class ListeActivity extends DefaultActivity implements CallbackObject {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// récupère la liste des article si elle a été conservée par une
		// précédente instance
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				if (headerLink != null) {
					...
					if (position == 0) {
						// Soit on a cliqué dessus, auquel cas on lance l'URI
						// spécifiée
						startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(headerLink)));
						return;
					} else
						position--; // Soit on décale les offsets pour accéder à
									// l'article correct.
				}

				if (position < listeArticles.size())// Vérifier que l'on n'a pas
													// cliqué sur le throbber
				{
		...
		// initialisation du listener de scroll pour catcher la fin de liste
		// atteinte
	...
	/**
	 * Essaye d'afficher plus d'éléments dans la liste (appel d'un autre
	 * thread).
	 * 
	 * @return échec si true, de nouveaux articles sont certainement déjà en
	 *         train d'être chargés
	 */
	protected boolean tryExpandListView() {
		...
		if (!echec) {
			...
			String url = baseUrl + "?start=" + prochainArticleATelecharger + "&limit="
					+ ARTICLES_A_CHARGER;
	...
	/**
	 * Ajoute les nouveaux articles dans la liste des articles. Si aucun nouvel
	 * article, on désactive l'ajout le chargement de nouveaux articles. Puis
	 * crée/notifie l'adapter de la liste.
	 */
		...
		public void run() {
			...
			try {
				...
				if (nouveauxArticles.size() < ARTICLES_A_CHARGER) {
					// plus d'articles à récupérer : on désactive le chargement
					// d'articles
				...
				// indique à l'adapter qu'il faut faire un resfresh UI du
				// contenu de la liste
	 ...
	 * 
	 * @param objet
	 *            JSON récupéré contenant la liste des articles
	 */
