...
public class ArticleObject implements Serializable {
	...
	/**
	 * Liste des sources. Une correspondance d'index est effectuée avec
	 * sourcesUrl
	 */
	 ...
	 * 
	 * @param jsonDatas
	 *            du json au format {Titre, Accroche?, Banniere?, Omnilogisme,
	 *            Sources:[]}
	 */
	public void remplirDepuisJSON(JSONObject jsonDatas) {
		try {
			 ...
			 * 
			 * Ces composants nécessitent de vérifier qu'ils ne sont pas nuls ou
			 * vides.
			 */
			...
			if (!jsonDatas.isNull("U")) {
				...
				for (int i = 0; i < sources.length(); i++) {
					...
					// Attention, si la source n'a pas été téléchargée depuis le
					// serveur, son titre est
					// vide.
					// Dans ce cas, on reprend directement l'URL.
					if (sources.getJSONObject(i).isNull("Titre")
							|| sources.getJSONObject(i).getString("Titre").trim().equals(""))
