...
public class AuteursActivity extends SpecialActivity implements CallbackObject {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				String nomAuteur = ((TextView) view
						.findViewById(R.id.item_auteurs_auteur_nomAuteur)).getText().toString();
	...
	protected Runnable remplirUIAvecDatas = new Runnable() {
		...
		public void run() {
			...
			ListAdapter adapter = new SimpleAdapter(auteursActivity, auteurs,
					R.layout.item_auteurs_auteur, new String[] { "Auteur", "NombreArticle" },
					new int[] { R.id.item_auteurs_auteur_nomAuteur,
							R.id.item_auteurs_auteur_nbArticles });
	 ...
	 * 
	 * @param objet
	 *            JSONArray récupéré contenant la liste des auteurs
	 */
				...
				for (int i = 0; i < jsonDatas.length(); i++) {
					...
					try {
						...
						auteur.put("NombreArticle", data.getString("N")
								+ (data.getString("N").equals("1") ? " article publié"
										: " articles publiés"));
